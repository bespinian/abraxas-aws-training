...
resource "aws_cloudwatch_log_group" "trail_logs" {
  ...
  name     = "/aws/cloudtrail/org-cloudtrail"
  retention_in_days = 90
}


# CloudWatch Log Metric Filter for Root User
resource "aws_cloudwatch_log_group" "trail_logs" {
  name     = "/aws/cloudtrail/org-cloudtrail"
  retention_in_days = 90
}
...
resource "aws_cloudtrail" "org_trail" {
  ...
  cloud_watch_logs_group_arn  = "${aws_cloudwatch_log_group.trail_logs.arn}:*" # -> Add this line
  cloud_watch_logs_role_arn   = aws_iam_role.cloudtrail_cloudwatch_role.arn # -> Add this line
...
resource "aws_cloudwatch_log_metric_filter" "root_user_filter" {
...
# Alarm on root activity
resource "aws_cloudwatch_metric_alarm" "root_user_alarm" {
  alarm_name          = "RootUserActivityAlarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  metric_name         = aws_cloudwatch_log_metric_filter.root_user_filter.metric_transformation[0].name
  namespace           = aws_cloudwatch_log_metric_filter.root_user_filter.metric_transformation[0].namespace
  period              = 300
  statistic           = "Sum"
  threshold           = 1

  alarm_description   = "Triggered when a Root user performs any activity"
  alarm_actions       = [aws_sns_topic.root_alerts.arn]
}

############################
# Root User Alert Setup
############################
